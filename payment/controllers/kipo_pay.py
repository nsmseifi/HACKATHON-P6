...
from financial_transactions.controller import transaction_controller as transaction_controller
from helper import Http_error, value, model_to_dict
...
def pay_by_kipo(data, db_session, username):
    ...
    if person_id is None:
        ...
        try:
            person = person_controller.get(person_id, db_session)
        except:
            logger.error(LogMsg.PERSON_NOT_EXISTS, {'person_id': person_id})
            raise Http_error(404, Message.Invalid_persons)

    validate_permissions_and_access(username, db_session, 'PAYMENT_ADD',
                                    per_data)
    ...
    if kpg_initiate['status']:
        ...
        payment = payment_controller.add_payment(data, db_session, username)
...
def receive_payment(db_session, **kwargs):
    ...
    payment = payment_controller.get(shopping_key, db_session)
    if payment is None:
        logger.error(LogMsg.PAYMENT_NOT_FOUND, shopping_key)
        raise Http_error(402, Message.INVALID_SHOPPING_KEY)
    if payment.used:
        logger.error(LogMsg.PAYMENT_ALREADY_USED, payment.to_dict())
        raise Http_error(402, Message.PAYMENT_ALREADY_CONSIDERED)

    logger.debug(LogMsg.PAYMENT_ENTITY_GET, payment.to_dict())
    ...
    if status is not None and status[0] == '1':
        ...
        # TODO transaction def change
        transaction = transaction_controller.internal_add(transaction_data, db_session)
        logger.debug(LogMsg.TRANSACTION_ADDED, model_to_dict(transaction))
        payment_update_data = dict(used=True, reference_code=inquiry.get('referent_code'),
                                   status='ReceivedFromBank')
        payment = payment_controller.edit_by_model(payment, payment_update_data,
                                                   db_session, permission_checked=True)

        logger.debug(LogMsg.PAYMENT_UPDATED_TO_USED, payment.to_dict())

        return HTTPResponse(status=302,
                            headers=dict(location=call_back_url + '/successful'))
